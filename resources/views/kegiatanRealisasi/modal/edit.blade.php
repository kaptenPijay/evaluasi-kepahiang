...
<div class="modal fade modalForm modalSingleCol" id="ModalEdit" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content large-shadow">
            ...
            {{-- Modal body --}}
            <form action="{{ route('kegiatanRealisasi.update') }}" method="POST" id="formEdit">
                @csrf
                <div class="modal-body"
                    ...
                    <div class="form-group">
                        ...
                        <select name="kegiatan_id" class="form-select" id="kegiatan_id">
    ...
    <script>
        $(document).ready(function () {
            $('#kegiatan_id').change(function () {
                let kegiatan_id = $(this).val();

                if (kegiatan_id) {
                    $.ajax({
                        url: `{{ route('kegiatanRealisasi.dataProgram', '') }}/${kegiatan_id}`,
                        type: "GET",
                        success: function (response) {
                            if (response.indikatorData  && response.indikatorData.length > 0) {
                                ...
                                $('#indikator_id').change(function (e) {
                                    ...
                                    const { target, tw_1, tw_2, tw_3, tw_4, satuan } = filteredTarget.realisasi_kegiatan
                                    let today = new Date();
